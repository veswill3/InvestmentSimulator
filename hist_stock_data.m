...
for i = 1:length(tickers)
    ...
    if status
        ...
        stocks(idx).Close = flipud(str2double(cl));     % save closing price data
        stocks(idx).Volume = flipud(str2double(volume));      % save volume data
        stocks(idx).AdjClose = flipud(str2double(adj_close)); % save adjustied close data
